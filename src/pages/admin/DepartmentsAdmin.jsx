...
const DepartmentsAdmin = () => {
  ...
  const openCreate = () => { setEditItem(null); reset({ name: '', description: '', icon: 'Heart' }); setModalOpen(true); };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (editItem) {
        await updateDocument(COLLECTIONS.DEPARTMENTS, editItem.id, data);
        await logActivity('update', COLLECTIONS.DEPARTMENTS, editItem.id, user.email);
        toast.success('Department updated');
      } else {
        const id = await createDocument(COLLECTIONS.DEPARTMENTS, data);
        await logActivity('create', COLLECTIONS.DEPARTMENTS, id, user.email);
  ...
  const handleDelete = async () => {
    try {
      await deleteDocument(COLLECTIONS.DEPARTMENTS, deleteId);
  ...
  return (
    <>
      <SEO title="Manage Departments" />
      <div className="flex justify-between items-center mb-8">
        <div><h1 className="font-display text-2xl font-bold">Departments</h1><p className="text-slate-500">Manage clinic departments</p></div>
        <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add Department</Button>
      </div>
      <DataTable columns={columns} data={departments} loading={loading} emptyTitle="No departments" />
      ...
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} message="Delete this department?" danger />
    </>
  );
};
